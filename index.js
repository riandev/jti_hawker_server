...
const app = express();
app.use(express.static("../jti_hawker_client/build"));
app.use(cors());
...
// const MongoClient = require("mongodb").MongoClient;
const uri = "mongodb://127.0.0.1:27017/jti_hawker";
// const uri =
//   "mongodb+srv://aktcl:01939773554op5t@cluster0.9akoo.mongodb.net/jti_hawker?retryWrites=true&w=majority";
const client = new MongoClient(uri, {
  useNewUrlParser: true,
  useUnifiedTopology: true,
});
client.connect((err) => {
  const userCollection = client.db("jti_hawker").collection("users");
  const adminCollection = client.db("jti_hawker").collection("admins");
  const leadsCollection = client.db("jti_hawker").collection("leads");
  ...
  app.get("*", (req, res) => {
    res.sendFile(
      path.join(__dirname, "../jti_hawker_client/build", "index.html")
    );
  });
});
